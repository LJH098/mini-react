...
description: Create a polished Korean GitHub issue from rough user notes, open it with gh, create and switch to a matching feature-#123 branch, and continue the implementation there. Use when the user wants an issue-first workflow driven by GitHub CLI instead of manually drafting the issue and branch.
---
...
- optional branch prefix, default `feature`
...
설명 초안:
Repo override(선택):
브랜치 prefix(기본값: feature):
Assignee(기본값: @me):
```

If the user already supplied rough notes in free form, infer the fields instead of forcing the template.
...
The script creates a branch named exactly `feature-#<issue-number>`.
...
- `scripts/create_issue_and_branch.py`: Create the issue with `gh`, extract the issue number, validate the `feature-#<issue>` branch name, and switch to the new branch.
